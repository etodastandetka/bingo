/**
 * Функции для пополнения баланса через API казино
 * Экспортируются для использования в email-watcher и других модулях
 */
import { prisma } from './prisma'
import { depositCashdeskAPI, depositMostbetAPI, deposit1winAPI } from './casino-deposit'

// Функция для получения конфигурации API казино из настроек
export async function getCasinoConfig(bookmaker: string) {
  const normalizedBookmaker = bookmaker?.toLowerCase() || ''
  
  // Для 1xbet, Melbet, Winwin, 888starz, 1xCasino и BetWinner нужны: hash, cashierpass, login, cashdeskid
  ...
  if (normalizedBookmaker.includes('melbet') || normalizedBookmaker === 'melbet') {
    ...
    // Fallback на дефолтные значения
    return {
      hash: process.env.MELBET_HASH || 'e926a363ccb63af5348d5e8154fdaf07795224ea551eeaeab5e5ebe0511ffefa',
      cashierpass: process.env.MELBET_CASHIERPASS || 'pUdKHv4SoV',
      login: process.env.MELBET_LOGIN || 'bakhtark',
      cashdeskid: process.env.MELBET_CASHDESKID || '1350588',
    }
  }

  // Для Winwin нужны: hash, cashierpass, login, cashdeskid
  ...
  // Для Mostbet/MBCash нужны: api_key, secret, cashpoint_id, x_project, brand_id
  ...
  if (normalizedBookmaker.includes('1win') || normalizedBookmaker === '1win') {
    ...
    // Fallback на дефолтные значения (API ключ из примера пользователя)
    return {
  ...
  // Для 888starz нужны: hash, cashierpass, login, cashdeskid
  ...
  // Для BetWinner нужны: hash, cashierpass, login, cashdeskid
  if (normalizedBookmaker.includes('betwinner') || normalizedBookmaker === 'betwinner') {
    // Сначала пробуем получить из БД
    const setting = await prisma.botConfiguration.findFirst({
      where: { key: 'betwinner_api_config' },
    })
  ...
  // Для WowBet нужны: hash, cashierpass, login, cashdeskid
  if (normalizedBookmaker.includes('wowbet') || normalizedBookmaker === 'wowbet') {
    // Сначала пробуем получить из БД
    const setting = await prisma.botConfiguration.findFirst({
      where: { key: 'wowbet_api_config' },
    })
