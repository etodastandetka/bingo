...
export async function getCasinoConfig(bookmaker: string) {
  ...
  if (normalizedBookmaker.includes('mostbet') || normalizedBookmaker === 'mostbet') {
    try {
      ...
      if (setting) {
        console.log(`[Mostbet Config] Found setting in DB, value type: ${typeof setting.value}`)
        let config: any
        try {
          config = typeof setting.value === 'string' ? JSON.parse(setting.value) : setting.value
          console.log(`[Mostbet Config] Parsed config:`, JSON.stringify(config).substring(0, 200))
        } catch (parseError: any) {
          console.error(`[Mostbet Config] Failed to parse config:`, parseError.message)
          config = setting.value
        }
        
        // Поддерживаем оба варианта: cashpoint_id и cashpointId, api_key и api, x_project и project
        ...
        console.log(`[Mostbet Config] Extracted values: apiKey=${!!apiKey}, secret=${!!secret}, cashpointId=${cashpointId}, project=${project}`)
        ...
        if (apiKey && secret && cashpointId) {
          const result = {
            ...
            x_project: String(project),
            brand_id: parseInt(String(config.brand_id || 1)),
          }
          console.log(`[Mostbet Config] ✅ Successfully loaded from DB: api_key=${result.api_key.substring(0, 25)}..., cashpoint_id=${result.cashpoint_id}, project=${result.x_project}`)
          return result
        } else {
          console.warn(`[Mostbet Config] ⚠️ Config found in DB but missing required fields: apiKey=${!!apiKey}, secret=${!!secret}, cashpointId=${cashpointId}`)
          console.warn(`[Mostbet Config] Full config object:`, JSON.stringify(config))
        }
      } else {
        console.warn(`[Mostbet Config] ⚠️ Config not found in DB (key: mostbet_api_config)`)
      }
    } catch (dbError: any) {
      console.error(`[Mostbet Config] ❌ Database error:`, dbError.message)
    }
    ...
    console.log(`[Mostbet Config] 🔄 Using fallback config: api_key=${fallbackConfig.api_key.substring(0, 25)}..., cashpoint_id=${fallbackConfig.cashpoint_id}`)
